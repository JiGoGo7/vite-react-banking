                    ...
                    {credits
                        ...
                        .map((credit) => (
                            <div key={credit._id} className="credit-card">
                                <p>
                                    <span className="credit-label">Користувач:</span> {credit.fullName}
                                </p>
                                <p>
                                    <span className="credit-label">Вік:</span> {credit.age}
                                </p>
                                <p>
                                    <span className="credit-label">Одружений:</span>{' '}
                                    {credit.maritalStatus ? 'Так' : 'Ні'}
                                </p>
                                <p>
                                    <span className="credit-label">Влаштований на роботу:</span>{' '}
                                    {credit.job ? 'Так' : 'Ні'}
                                </p>
                                <p>
                                    <span className="credit-label">Заробітня плата:</span> {credit.income}₴
                                </p>
                                <p>
                                    <span className="credit-label">Місячні витрати:</span> {credit.expenses}₴
                                </p>
                                <p>
                                    <span className="credit-label">Запитана сума:</span> {credit.requestedAmount}₴
                                </p>
                                <p>
                                    <span className="credit-label">Кредитний рейтинг користувача:</span>{' '}
                                    {rating ? rating[credit._id] ?? 'Немає даних' : 'Завантаження...'}
                                </p>
                                <div className="credit-actions">
                                    <button className="approve-btn" onClick={() => handleApprove(credit._id)}>
                                        Схвалити
                                    </button>
                                    <button className="reject-btn" onClick={() => handleReject(credit._id)}>
                                        Відхилити
                                    </button>
                                </div>
                            </div>
